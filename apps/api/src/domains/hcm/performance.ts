...
export async function listOneOnOnes(filters: { employmentRelationshipId?: string }) {
  const auth = currentAuth();
  await assertCan({ resource: 'hcm_one_on_ones', verb: 'view' });
  const scope = await scopeFor('hcm_one_on_ones', 'view');
...
export async function scheduleOneOnOne(input: {
  ...
  const auth = currentAuth();
  await assertCan({ resource: 'hcm_one_on_ones', verb: 'create' });
  ...
  const scope = await scopeFor('hcm_one_on_ones', 'create');
...
export async function updateOneOnOne(
  ...
  await assertCan({ resource: 'hcm_one_on_ones', verb: 'edit', record: { ownerPartyId: row.reportPartyId } });

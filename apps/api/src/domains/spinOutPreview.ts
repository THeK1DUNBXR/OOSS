/**
 * The read-only spin-out preview (equity-portal plan §6b) — the same preview
 * `pnpm division:spin-out` prints, scoped to the caller's own tenant. Kept
 * apart from `group.ts` so the group read model stays a snapshot reader and
 * its source test (EQT-GRP-001) keeps meaning what it says.
 *

...
export async function listBoardMembers() {
  ...
  const fyEnd = fyEndFromProfile(profile);
...
/**
 * The financial year end as a date carrying only month and day. The profile
 * records the month (`financialYearEndMonth`, default March); the day is the
 * last day of that month, which is what the Act means by "31 March".
 */
function fyEndFromProfile(profile: { financialYearEndMonth: number | null } | null): Date {
  const month = profile?.financialYearEndMonth ?? 3;
  // Day 0 of the next month is the last day of this one.
  return new Date(Date.UTC(2000, month, 0));
}
...
async function sweepMbp1(profile: { financialYearEndMonth: number | null } | null): Promise<number> {
  const auth = currentAuth();
  const now = new Date();
  const fyEnd = fyEndFromProfile(profile);
...
async function sweepAgmDue(profile: { financialYearEndMonth: number | null; incorporatedOn: Date | null } | null): Promise<number> {
  if (!profile) return 0;
  const auth = currentAuth();
  const now = new Date();
  const { start, end: fyEnd } = financialYearWindow(fyEndFromProfile(profile), now);

...
describe("EQT-BRD-007 — the compliance job writes each item once and reads 'No meeting recorded' rather than overdue on an empty register", () => {
  it('an empty register raises nothing; once incorporation is on record the job raises one item, never twice', async () => {
    ...
    // The first-meeting item, and — because the profile carries a financial
    // year end (March by default) — the AGM item for the first financial year
    // as well. The kind-specific assertions below pin the property that
    // matters: the first-meeting item exists exactly once.
    expect(raisedOnce).toBeGreaterThanOrEqual(1);

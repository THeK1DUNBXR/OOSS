...
cd apps/api && pnpm test      # 749 tests
```
...
| **EQT-REG** Equity portal — Phase 1 the register | 10 | The share ledger has no update route and a reversal posts the opposite entry rather than editing the original; distinctive numbers are assigned gaplessly per class and never overlap across two allotments; the secretary holds no `share_ledger:approve` grant at all and the chairman is refused (self-dealing bar) approving a transfer where they are the recipient; the cap table's issued% and fully-diluted% bases each sum to exactly 100.00 (largest-remainder rounding) and a priced allotment with no `considerationTransactionId` is flagged `pendingConsideration`; a shareholder's `holdings/me` resolves their own holder and another holder's id is a 404; an issued certificate's printed-document view is unchanged (deep-equal) after a later transfer of unrelated shares; the two-month SH-1 window (`EX-EQT-002`) is raised once per overdue transaction and not duplicated on a second run; a holder recorded in the holding's own register naming a subsidiary as `heldByTenantId` is refused (s.19); a non-resident holder with no stated investment basis is refused; the opening-register import creates a share class, a holder and an effective allotment carrying the given certificate number, and revert removes the transaction and its certificate cleanly |
| **EQT-GRP** Equity portal — Phase 2 the group | 9 | A source grep confirms `domains/group.ts` calls `unscopedPrisma` only inside the marked publisher section, and holds no literal role-slug comparison; the worked look-through example (founder 60% of the holding × the holding's 70% of a subsidiary, plus the founder's own 5% direct) comes to exactly 47%, on both bases, and a cycle in the ownership graph is refused rather than computed; a snapshot older than 24 hours is labelled `stale` with its age in seconds and its figures stay a distinct row, never folded into another entity's; a holder named two hops below the top holding (`kaizen` → parent → child) is refused as a holder in the top holding's own register (s.19, extending `EQT-REG-008`'s one-hop case), while a holding recorded as a holder two layers down in the grandchild's own register is the legitimate direction and succeeds; a chairman with no active affiliation in a subsidiary still reads its published snapshot from the group screen and is refused (404) a `switch-entity` into it; `groupFinancials()` carries the heading "Group total before inter-company eliminations" verbatim and lists the inter-company amounts a `Transaction.intercompanyTenantId` outside the caller's own group is refused (422) at entry; a division with no subsidiary tenant yet reads `not_yet_incorporated` on the structure chart and stops the moment a tenant publishing with that `originDivision` exists |
| **EQT-RND** Equity portal — Phase 4 rounds, instruments, valuations, scenarios | 9 | A preferential round refuses to open with no valuation, with a wrong-basis valuation, and with a future-dated report, and opens once a past `registered_valuer` report is on file (Rule 13); a private placement opens at exactly 200 offerees across a financial year's rounds and refuses the 201st (s.42); a 1:2 conversion cancels exactly the convertible holding's own count and allots double in the target class, both bases tracked correctly through the ledger's usual balance and cap-table arithmetic; a bonus round with `sourceOfBonus: free_reserves` refuses to open when the books carry no transaction to compute free reserves from, and says so rather than treating the absence as zero; a buy-back above the 10% board ceiling with no shareholder resolution on the round is refused by name, and a buy-back well inside that ceiling is still refused once debt-to-equity exceeds 2:1; closing a round raises `EX-EQT-003` with a due date 15 days out for a private placement and 30 for every other kind; `modelRound` dilutes every existing holder by the same proportion of the pre-round total and `waterfall` pays a non-participating preference before common at a low exit and lets the holder convert instead at a high one; the round-model and waterfall endpoints read the live cap table and write nothing to the register |
| **EQT-ESP** Equity portal — Phase 5 ESOP | 10 | A grant over the pool's available options is refused as an over-allocation; a schedule whose cliff is under twelve months is refused on the plan default and on a grant's own override alike (Rule 12); a >10% holder is refused a grant under a non-DPIIT plan and the same grant succeeds under a DPIIT-recognised one, with `promoterCheck.dpiitReliefApplied` set; the proposer of a grant and its own grantee are both refused approving it before the gate is even consulted; the daily vesting job marks a due tranche vested exactly once and does not vest a tranche whose employment has since ended; an exit lapses an unvested tranche immediately, leaves a vested-unexercised balance alone with a fixed `exerciseWindowEndsOn`, and the daily sweep lapses that balance once the window has passed; an approved exercise allots the target class through the same `makeEffective` phase 1 built, at the grant's own exercise price, with the `ShareTransaction` linked back onto the exercise; a perquisite is computed only from a merchant-banker `Valuation` recorded within 180 days and is withheld with a named reason otherwise; the SH-6 register lists every grant oldest-first and the same rows export as a parseable `.xlsx`; an employee's own-grants view and the scoped list endpoint both return only their own grants |
...
| **MKT-INT** Marketing — pluggable channel adapters | 20 | `marketingAdapters.test.ts`. Merge-field rendering substitutes dotted paths, reports missing fields, escapes HTML only when asked, and never executes a malicious-looking field as code; every channel defaults to `not_configured` with no env set; the console, MSG91 (India DLT) and WhatsApp Cloud adapters each refuse to send without their own required template registration even when otherwise configured; every inbound webhook parser returns `[]` on a garbage payload rather than throwing |
| **MKT-CMP / MKT-BUD** Marketing — campaigns, approval and budget | 17 | `marketingCampaigns.test.ts`. A campaign starts in draft; submit resolves to `scheduled` or `pending_approval` by the tenant's own threshold; the self-dealing bar holds even for the chairman — a proposer, including the chairman, can never approve their own campaign, and `finance_head` approves and rejects what it did not propose; only `CAMPAIGN_TRANSITIONS`-listed moves are accepted through the full launch/pause/resume/complete lifecycle; delete is draft-only and soft; edit is refused once a campaign has left draft/paused/scheduled; a campaign is not found from another tenant by direct id; budget variance is not measured (never `0`) without an approved budget, and spend exceeding the planned amount raises `EX-MKT-002` |
| **MKT-AUD / MKT-CON** Marketing — audiences and consent | 14 | `marketingAudiences.test.ts`. A dynamic audience evaluates to exactly the rule's matching leads; static add/remove keeps `memberCount` correct both ways; a suppression audience excludes its members from `audiencePersonIds`; the send-eligibility gate skips a person with no marketing consent, a person marked do-not-contact even with consent granted, and a person opted out of the specific channel, reading the existing `Consent` table live rather than a cache; an unsubscribe token round-trips to the one person it was issued for and an unrecognised token withdraws nobody; consent coverage reads not-measured with nobody ever contacted; the 15 default channels seed idempotently; cross-tenant isolation on audiences |
| **MKT-MSG** Marketing — templates, sends and journeys | 11 | `marketingMessaging.test.ts`. Editing an approved template opens a new draft version and retires the old one on approval; an SMS template cannot be approved without a `dltTemplateId`; a template's own creator can never approve it (self-dealing); a send lists consented recipients as queued and everyone else as skipped with a named reason; requesting a send with no configured adapter raises `EX-MKT-011` and leaves it in draft rather than silently succeeding; dispatch with the console adapter marks recipients sent and logs an Interaction, and a bounce rate over the alarm threshold raises `EX-MKT-005`; a send's own requester can never also approve it; `tick()` enrols and advances a journey run by one step at a time and enrolling the same person twice never duplicates the active run; cross-tenant isolation on templates/sends |
| **MKT-CAP** Marketing — forms, capture and lead scoring | 22 | `marketingCapture.test.ts`. A public submission is accepted only against an active `publicToken` and 404s against an unpublished form or an unknown token; a converted submission resolves or creates a Person via `findOrCreatePerson` and a Lead carrying campaign/channel/utm, with a second identical email never forking identity; the honeypot `website` field silently records the submission as spam while still answering `ok:true`; an identical payload within 10 minutes is marked a duplicate rather than converted twice; the 31st submission from one IP in a minute is rejected with 429 and a different IP is unaffected; a submission unconverted for 24h raises `EX-MKT-007`; `recordTouchpoint` exposes no update or delete path — the log is immutable; a short-link click records a touchpoint, appends utm to the target and increments `clickCount`; lead score is the sum of active rule matches with every CRM scoring reason kept and `mkt:` reasons only appended on top; a lead with no attributable source raises `EX-MKT-001` |
| **MKT-EVT / MKT-AST** Marketing — events, registrations and assets | 21 | `marketingEvents.test.ts`. The event state machine moves `planned → open → closed → live → completed` and rejects an illegal jump; `institutionId` must resolve to an in-tenant institution Organization; registration enforces capacity, rejects a duplicate and is only accepted while the event is open; check-in records an `event_attend` touchpoint and a completed follow-up logs a meeting Interaction against the person; `convertAttendees` creates a lead (source `event`) for each attended registration lacking one, and cost-per-attendee is not measured while nobody has attended; a completed event with an unresolved follow-up after 3 days raises `EX-MKT-008`; asset versioning opens a new draft row at version+1 on edit and bars the creator from approving their own asset; an approved, expired asset still referenced by a scheduled social post raises `EX-MKT-012`; publishing a social post with no configured adapter records `publishMode: manual` and requires an `externalUrl` |
| **MKT-ANA / MKT-GOV** Marketing — analytics, health and AI drafts | 16 | `marketingAnalytics.test.ts`. Funnel conversion rates are `null`, never zero, when the stage above has zero denominator, and real once touchpoints and an attributed lead exist; channel cost-per-lead is `null` when a channel has spend but zero leads; the overview always returns exactly the documented KPI keys, each carrying a `measured` flag, and CSV export is text/csv-shaped with a header row; `H_MKT` reports `not_yet_measured` while no campaign has ever gone live and a scored band with a drill path on every factor once one has; all seven marketing jobs are registered, runnable individually and as a group on an empty tenant; every `AI-MKT` draft kind registers an `AgentAction` at its own DRAFT/RECOMMEND tier and never sends, creates or approves anything itself; the `marketing-assistant` agent registers idempotently and is never granted a write tool |
